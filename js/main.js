...
function getChoroplethColor(featureProperties, colorScale){
  // convert val to float for expressed global variable
  var val = parseFloat(featureProperties[expressed]);
...
function setChart(csvData, colorScale) {
  ...
  var chartTitle = chart.append("text")
      ...
      .text("State " + attArrayTranslate[expressed]);
...
function updateChart(bars, n, colorScale) {
  ...
  var chartTitle = d3.select(".chartTitle")
      .text(attArrayTranslate[expressed]);
...
function setLabel(dataProperties) {
  ...
  if (expressed != "") {
      var val =
          parseFloat(Math.round(dataProperties[expressed] * 100) / 100).toFixed(2);
  } else {
      val = dataProperties[expressed];
  }

  // label content for specific attribute, accessed through a dictionary
  var labelAttribute = "<h1>" + val +
      "</h1><b>" + attArrayTranslate[expressed] + "</b>";
